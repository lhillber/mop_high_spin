...
class Simulation(IO):
    ...
    def process_timing(self):
        if self.delay is None:
            vv = self.cloud.init_macrostate["V"][0]
            if vv == 0.0:
                self.delay=0
            else:
                self.delay = np.abs(self.cloud.init_macrostate["R"][0] / vv)
    ...
    def plot_current(self, axs=None, **kwargs):
        ...
        for n, pulse in enumerate(self.pulses):
            ...
            for Iname, I0 in Idic.items():
                ...
                Is = I0 * pulse["Is"]
                axs.scatter(ts, Is, label=label, **kwargs)
        axs.set_xlabel("t [$\mu$s]")
        axs.set_ylabel("I [A]")
        plt.legend(ncol=4, handletextpad=0.1)
        fig.tight_layout()
        return fig, axs
    ...
    def plot_measure(
        ...
        for name, mdata, color, line in zip(names, data.T, colors, lines):
            if name not in blacklist:
                if logy:
                    axs.semilogy(ts, mdata, label=name, c=color, ls=line, ms=2)
                else:
                    axs.scatter(ts, mdata, label=name, c=color, ls=line, s=2)
    ...
    def plot_measures(self):
        ...
        for measure_name in self.to_measure:
            if measure_name not in ("xs", "vs"):
                fig, ax = self.plot_measure(measure_name)
                axs.append(ax)
        return fig, axs

    def plot_phasespaces(
        self, plot_idx=[0, -1], remove_mean=False, Nsample=10000
    ):
    ...
    def plot_trajprojections(self, axs=None, N=10, color=None):
